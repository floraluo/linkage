+(function($){
	...
	Linkage.prototype.init = function (type, element, options) {
		...
		this.$tab      = this.$element.find('.linkage-tab')
	...
	Linkage.prototype.setProv = function() {
		...
		$.each( this.options.url , function(i , prov) {
			html+= '<li data-index='+i+' data-value='+options.formatProvResult(prov).value+'>'+ options.formatProvResult(prov).template+'</li>'
		})
	...
	Linkage.prototype.setTab = function () {
		...
		var options = this.options,
				provs = options.url,
				citys = options.getCitys(provs[0]),
				dists = options.getDists(citys[0]),
				prov_tab = this.options.prov ? this.options.prov : options.formatProvResult(provs[0]).tab,
				city_tab = this.options.city ? this.options.city : options.formatCityResult(citys[0]).tab
		 this.$tabvalue.eq(0).html(prov_tab)
		 this.$tabvalue.eq(1).html(city_tab)

		if(!typeof(dists) == 'undefined'){
			dist_tab = this.options.dist ? this.options.dist : options.formatDistResult(dists).tab
	...
	Linkage.prototype.setCity = function(index) {
		...
		var html = "", 
				...
				provs = options.url,
				citys =options.getCitys(provs[prov_index])

		if(prov_index < 0||typeof(citys)=="undefined") return

		$.each( citys,function(i,city){
			html+="<li data-index='"+i+"' data-value='"+options.formatCityResult(city).value+"'>"+ options.formatCityResult(city).template +"</li>"
		})
	...
	Linkage.prototype.setDist = function(index) {
		...
		var html = "", 
				...
				provs = options.url,
				citys = options.getCitys(provs[prov_index]),
				dists = options.getDists(citys[city_index])

		if(city_index < 0||typeof(dists)=="undefined") return
			
		$.each( dists,function(i,dist){
			html+="<li data-index='"+i+"' data-value='"+options.formatDistResult(dist).value+"'>"+ options.formatDistResult(dist).template +"</li>"
		})
	...
	Linkage.prototype.selectValue = function (e) {
		var $nextTab , inputValue='' , i =0 , 
				...
				provs = options.url,
				citys = options.getCitys(provs[thisObj.index]),
				dists = options.getDists(citys[0])

		this.$tabvalue.eq(panelIndex).html(thisObj.value).attr('data-index',thisObj.index)
		if( panelIndex < tabCount-1 ) { 
			// 除最后一个面板之外，选择前面面板需要设置tab
			$nextTab = this.$tabvalue.eq(thisObj.panelIndex+1)
			if(panelIndex == 0) {
				// 第一个面板需要设置省市县
				$nextTab.html( options.formatCityResult(citys[0]).value )
				var $distTab = this.$tabvalue.length == 3 ? this.$tabvalue.eq(2) : null
				if( $distTab != null ){
					var dist_tab_value = dists ? options.formatDistResult(dists[0]).value : ""
					$distTab.html(dist_tab_value)
				}
			}
			if(panelIndex == 1) {
				// 倒数第二个面板即市级面板选择之后，直辖市可能没有县级城市的情况
				var prov_index = this.$tabvalue.eq(0).attr('data-index'),
						spe_citys = options.getCitys(provs[prov_index]),
						spe_dists = options.getDists(spe_citys[thisObj.index]),
						dist = spe_dists ? options.formatDistResult(spe_dists[0]).value : ""
				if(dist == ""){
					tabCount--
				}else{
					$nextTab.html(dist)					
  ...
  $(document).on('click.fl.linkage', function(e){
		if($(e.target).closest(".linkage").length > 0){
			e.stopPropagation()	
		}else{
  		Linkage.prototype.close()
		}
  })
})(jQuery);
