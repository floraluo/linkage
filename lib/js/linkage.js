+(function($){
	...
	var Linkage = function(element , options) {
		...
		// 	.on('click.linkage',  $.proxy(this.selectValue, this))
		this.$target = this.$element.find(".linkage-content li")
			.on('click.linkage',  $.proxy(this.selectValue, this))

		this.$target = this.$element.find('.linkage-input')
			.on('click.linkage', $.proxy(this.open, this))
	...
	Linkage.DEFAULTS = {
		...
		formatResult: function(result){
			return formatted = {
				value: result.p,
				template: '<span>'+result.p+'</span>'
			} 
		},
		formatTab: function(result) {
			return formatted = {
				template: result
			}
		}
	...
	// NOTE: Linkage EXTENDS tooltip.js
	// ================================

	// Linkage.prototype = $.extend({}, $.fn.tooltip.Constructor.prototype)
	
	// Linkage.prototype.constructor = Linkage
	...
	Linkage.prototype.init = function (type, element, options) {
		...
		this.setProv()
		this.setTab()
		// var tablist = this.$element.find('.value'),
		// 		preData = this.options.url[0],
		// 		valueArr = [ preData.p , preData.c[0].n , preData.c[0].a[0].s ];
		// tablist.each(function(i){
		// 	$(this).eq(i).html(valueArr[i]);
		// })
	...
	Linkage.prototype.setProv = function() {
		...
		$.each( this.options.url , function(i , prov) {
			html+= '<li data-index='+i+' data-value='+prov.p+'>'+prov.p+'</li>'
		})
	...
	Linkage.prototype.setCity = function(index) {
		var html = "",
				prov_id = index ? index : 0

		if(prov_id < 0||typeof(this.options.url[prov_id].c)=="undefined") return
			
		$.each( this.options.url[prov_id].c,function(i,city){
			html+="<li data-index='"+i+"' data-value='"+city.n+"'>"+city.n+"</li>";
		});
		this.$panel.eq(1).html("<ul class='area-list'>"+html+"</ul>");
		// this.setDist()
	}
	Linkage.prototype.setDist = function() {
